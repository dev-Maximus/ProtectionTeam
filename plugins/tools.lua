...
local text = matches[2]
tdcli.createNewChannelChat(text, 1, '@CoderTeam', (function(b, d) tdcli.addChatMember(d.id_, msg.from.id, 0, dl_cb, nil) end), nil)
   if not lang then 
...
tdcli.sendMessage(k, 0, 0, bc, 0)			
  ...
  if is_sudo(msg) then
	if (matches[1]:lower() == "sendfile" or matches[1] == 'ارسال فایل') and matches[2] and matches[3] then
		...
		tdcli.sendDocument(msg.chat_id_, msg.id_,0, 1, nil, send_file, '@CoderTeam', dl_cb, nil)
	end
	if matches[1]:lower() == "sendplug" or matches[1] == 'ارسال پلاگین' and matches[2] then
	    local plug = "./plugins/"..matches[2]..".lua"
		tdcli.sendDocument(msg.chat_id_, msg.id_,0, 1, nil, plug, '@CoderTeam', dl_cb, nil)
    end
  end
